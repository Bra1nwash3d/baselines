...
class DncPolicyPredictive2(object):
    ...
    def __init__(self, sess, ob_space, ac_space, nenv, nsteps, nstack, args, reuse=False):
        ...
        with tf.variable_scope("model", reuse=reuse):
            ...
            dnc_model = MaskedDNC(access_config, controller_config, args.get('ndncout', 64), args.get('clip_value', 200000))
            msc = tf.subtract(tf.ones_like(ms), ms, name='mask_sub')  # previously 1 means episode is over, now 1 means it continues
        ...
        def step(ob, state, mask):
            # action, value, agent state, obs description, obs prediction, early obs descr., decoded early obs descr.
            a, v, s, obs_desc, obs_pred, obs_enc, obs_dec = sess.run([a0, v0, snew, obs_description, obs_prediction,
                                                                     obs_encoded, obs_decoded],
                                                               {X:ob, S:state, M:mask})
            return a, v, s, obs_desc, obs_pred, obs_enc, obs_dec
...
class DncPolicyPredictive3(object):
    ...
    def __init__(self, sess, ob_space, ac_space, nenv, nsteps, nstack, args, reuse=False):
        ...
        def step(ob, state, mask):
            # action, value, agent state, obs description, obs prediction, early obs descr., decoded early obs descr.
            a, v, s, obs_desc, obs_pred = sess.run([a0, v0, snew, obs_description, obs_prediction],
                                                               {X:ob, S:state, M:mask})
            return a, v, s, obs_desc, obs_pred
...
class DncPolicyPredictive32(object):
    """
    This policy predicts the future observation that the hidden vector behind the conv layers will have.
        obs_description and obs_prediction belong to this part.
    This policy does not have the auto-encoder part and uses two layers to predict the observation delta.
    """
    def __init__(self, sess, ob_space, ac_space, nenv, nsteps, nstack, args, reuse=False):
        nlstm = args.get('nlstm', 64)
        nbatch = nenv*nsteps
