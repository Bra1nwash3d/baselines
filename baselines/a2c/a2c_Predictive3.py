...
class Model(object):
    ...
    def __init__(self, policy, policy_args, ob_space, ac_space, nenvs, nsteps, nstack, num_procs,
                 ent_coef=0.01, vf_coef=0.5, pred_coef=0.5, pred_reward_max=0.5,
        ...
        loss = pg_loss - entropy*ent_coef + vf_loss*vf_coef + pred_loss*pred_coef
        ...
        def train(obs, states, rewards, masks, actions, values, descriptions, predictions):
            ...
            policy_loss, value_loss, policy_entropy, prediction_loss, _ = sess.run(
                [pg_loss, vf_loss, entropy, pred_loss, _train],
                td_map
            )
            return policy_loss, value_loss, policy_entropy, prediction_loss
...
class Runner(object):
    ...
    def run(self):
        ...
        for n in range(self.nsteps):
            actions, values, states, obs_desc, obs_pred = self.model.step(self.obs, self.states, self.dones)
        ...
        return mb_obs, mb_states, mb_rewards, mb_masks, mb_actions, mb_values, mb_descs, mb_preds
...
def learn(policy, policy_args, env, seed, nsteps=5, nstack=1, total_timesteps=int(80e6),
          ...
          epsilon=1e-5, alpha=0.99, gamma=0.99, log_interval=100, pred_coef=0.5,
    ...
    model = Model(policy=policy, policy_args=policy_args, ob_space=ob_space, ac_space=ac_space, nenvs=nenvs,
                  ...
                  lrschedule=lrschedule, pred_coef=pred_coef)
    ...
    for update in range(1, total_updates):
        obs, states, rewards, masks, actions, values, descriptions, predictions = runner.run()
        policy_loss, value_loss, policy_entropy, prediction_loss =\
            model.train(obs, states, rewards, masks, actions, values, descriptions, predictions)
