...
class Model(object):
    ...
    def __init__(self, policy, policy_args, ob_space, ac_space, nenvs, nsteps, nstack, num_procs,
                 ent_coef=0.01, vf_coef=0.5, pred_coef=0.5, pred_reward_max=0.5, dec_coef=0.25,
                 max_grad_norm=0.5, lr=7e-4, alpha=0.99, epsilon=1e-5, total_timesteps=int(80e6), lrschedule='linear'):
        ...
        V = tf.placeholder(tf.float32, [nbatch])
        ...
        OBS_DESC = tf.placeholder(tf.float32, [nbatch, policy_args.get('nobsdesc')])
        OBS_PRED = tf.placeholder(tf.float32, [nbatch, policy_args.get('nobsdesc')])
        ...
        self.encoding_shape = [nbatch, step_model.encoding_shape()[-1]]
        OBS_EN = tf.placeholder(tf.float32, self.encoding_shape)
        OBS_DEC = tf.placeholder(tf.float32, self.encoding_shape)
        ...
        # clip vf and pred loss so that pg loss remains most important
        # pgl_p = tf.abs(pg_loss)
        # vf_loss = tf.clip_by_value(vf_loss, 0, pgl_p)
        # pred_loss = tf.clip_by_value(pred_loss, 0, pgl_p)
        ...
        def train(obs, states, rewards, masks, actions, values, descriptions, predictions, encoded, decoded):
            for step in range(len(obs)):
                cur_lr = lr.value()
            td_map = {train_model.X: obs, A: actions, V: values, R: rewards, LR: cur_lr,
                      OBS_DESC: descriptions, OBS_PRED: predictions, M: masks,
                      OBS_EN: encoded, OBS_DEC: decoded}
            if states != []:
                td_map[train_model.S] = states
                td_map[train_model.M] = masks
            policy_loss, value_loss, policy_entropy, prediction_loss, decoding_loss, _ = sess.run(
                [pg_loss, vf_loss, entropy, pred_loss, dec_loss, _train],
                td_map
            )
            return policy_loss, value_loss, policy_entropy, prediction_loss, decoding_loss
        ...
        self.value_and_obsdesc = step_model.value_and_obsdesc
...
class Runner(object):
    ...
    def __init__(self, env, model, description_size, nsteps=5, nstack=4, gamma=0.99):
        self.env = env
        self.model = model
        nenv = env.num_envs
    ...
    def run(self):
        ...
        for n in range(self.nsteps):
            actions, values, states, obs_desc, obs_pred, obs_enc, obs_dec = self.model.step(self.obs, self.states, self.dones)
        ...
        mb_masks = mb_dones[:, :-1]
        mb_dones = mb_dones[:, 1:]
        lv, lobs = self.model.value_and_obsdesc(self.obs, self.states, self.dones)
        mb_descs.append(lobs)  # removing first obs, adding one more, now obs[i] supposely matches pred[i]
        mb_descs.pop(0)
        mb_descs = np.asarray(mb_descs, dtype=np.float32).swapaxes(1, 0).reshape(self.batch_desc_shape)
        mb_preds = np.asarray(mb_preds, dtype=np.float32).swapaxes(1, 0).reshape(self.batch_desc_shape)
        mb_encs = np.asarray(mb_encs, dtype=np.float32).swapaxes(1, 0).reshape(self.model.encoding_shape)
        mb_decs = np.asarray(mb_decs, dtype=np.float32).swapaxes(1, 0).reshape(self.model.encoding_shape)
        last_values = lv.tolist()
        ...
        return mb_obs, mb_states, mb_rewards, mb_masks, mb_actions, mb_values, mb_descs, mb_preds, mb_encs, mb_decs


def learn(policy, policy_args, env, seed, nsteps=5, nstack=1, total_timesteps=int(80e6),
    ...
    runner = Runner(env, model, policy_args.get('nobsdesc'), nsteps=nsteps, nstack=nstack, gamma=gamma)

    nbatch = nenvs * nsteps
    tstart = time.time()
    total_updates = total_timesteps//nbatch+1
    for update in range(1, total_updates):
        obs, states, rewards, masks, actions, values, descriptions, predictions, encoded, decoded = runner.run()
        policy_loss, value_loss, policy_entropy, prediction_loss, decoding_loss =\
            model.train(obs, states, rewards, masks, actions, values, descriptions, predictions, encoded, decoded)
        if update % log_interval == 0 or update == 1:
            ...
            logger.record_tabular("loss_value", float(value_loss))
            logger.record_tabular("loss_prediction", float(prediction_loss))
            logger.record_tabular("loss_decoding", float(decoding_loss))
...
def play(policy, policy_args, env, seed, nep=5, save_path='', save_name='model'):
    ...
    if nep <= 0:
        ...
        for e in range(nep):
            ...
            episode_reward = 0
            while not done:
                actions, values, states, obs_desc, obs_pred, obs_en, obs_dec = model.step(observations, states, [done])
                #print('obs', new_obs)
                new_obs, reward, done, info = env.step(actions[0])
                #print('desc:', obs_desc)  # TODO remove
                #print('')
                #print('pred:', obs_pred)
                observations = update_obs(observations, new_obs)
