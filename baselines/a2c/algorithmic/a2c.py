...
class Model(object):
    ...
    def __init__(self, policy, policy_args, ob_space, ac_space, nenvs, nsteps, num_procs,
        ...
        A = tf.placeholder(tf.int32, [len(ac_space.spaces), nbatch])
        V = tf.placeholder(tf.float32, [nbatch])
        R = tf.placeholder(tf.float32, [nbatch])
        LR = tf.placeholder(tf.float32, [])
        adv = R - V

        step_model = policy(sess, ob_space, ac_space, nenvs, 1, policy_args, reuse=False)
        train_model = policy(sess, ob_space, ac_space, nenvs, nsteps, policy_args, reuse=True)

        pg_loss = tf.zeros(dtype=tf.float32, shape=[])
        for i in range(len(train_model.pi)):
            ...
            pg_loss += (tf.reduce_mean(adv * neglogpac) - entropy*ent_coef)
...
class Runner(object):
    ...
    def __init__(self, env, model, nsteps=5, gamma=0.99, step_alive_reward=0, episode_end_reward=0):
        self.env = env
        self.model = model
        nenv = env.num_envs
        self.obs_dtype = np.float32
        self.batch_ob_shape = (nenv*nsteps, 1)
        self.obs = np.zeros((nenv, 1), dtype=self.obs_dtype)
    ...
    def run(self):
        ...
        for n in range(self.nsteps):
            ...
            actions = corrected_actions(actions)
            ...
            self.states = states
            self.dones = dones
            for n2, done in enumerate(dones):
                if done:
                    self.obs[n2] = self.obs[n2]*0
            self.update_obs(obs)
        ...
        mb_actions = np.asarray(mb_actions, dtype=np.int32).swapaxes(2, 0)
...
def corrected_actions(actions):
    return [list(a) for a in np.array(actions).swapaxes(0, 1)]


def learn(policy, policy_args, env, env_args, seed, nsteps=5, total_timesteps=int(80e6), trained_timesteps=0,
    ...
    model = Model(policy=policy, policy_args=policy_args, ob_space=ob_space, ac_space=ac_space, nenvs=nenvs,
                  nsteps=nsteps, num_procs=num_procs, ent_coef=ent_coef, vf_coef=vf_coef,
...
def play(policy, policy_args, env, env_args, seed, nep=5, save_path='', save_name='model'):
    ...
    if nep <= 0:
        DNCVisualizedPlayer.player(env, model, nstack=1, env_args=env_args)
    else:
        total_reward = 0
        for e in range(nep):
            done = False
