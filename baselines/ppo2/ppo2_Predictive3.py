...
class Model(object):
    def __init__(self, *, policy, policy_args, ob_space, ac_space, nbatch_act, nbatch_train,
        ...
        def train(lr, cliprange, obs, returns, masks, actions, values, neglogpacs,
                  descriptions, predictions, states=None):
            ...
            return sess.run([pg_loss, vf_loss, entropy, approxkl, clipfrac, pred_loss, _train], td_map)[:-1]
        ...
        self.loss_names = ['loss_policy', 'loss_value', 'policy_entropy', 'approxkl', 'clipfrac', 'loss_prediction']
        self.train = train
    ...
    def run(self):
        ...
        for _ in range(self.nsteps):
            actions, values, self.states, neglogpacs,\
                obs_desc, obs_pred = self.model.step(self.obs, self.states, self.dones)
        ...
        return (*map(sf01, (mb_obs, mb_returns, mb_dones, mb_actions, mb_values, mb_neglogpacs,
                            mb_descs, mb_preds)), mb_states, epinfos)
